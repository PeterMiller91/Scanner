...
# - Vision direkt an OpenAI (openai>=1.0.0 Responses API)
...
def add_best_effort_dates(df: pd.DataFrame) -> pd.DataFrame:
    # receipt_date falls vorhanden, sonst created_at (UTC) als Datum
    df = df.copy()
...
def item_amount(row) -> float:
    # bevorzugt total_price, sonst qty * unit_price, sonst 0
...
def build_rollups(df: pd.DataFrame):
    ...
    # weekly: ISO week
    iso = df["day"].apply(lambda d: (d.isocalendar().year, d.isocalendar().week) if pd.notna(d) else (None, None))
    df["iso_year"] = [x[0] for x in iso]
    df["iso_week"] = [x[1] for x in iso]
...
if page == "Scan & Extrahieren":
    ...
    with c2:
        ...
        st.caption("Tipp: gpt-4.1 ist genauer, gpt-4.1-mini ist günstiger.")
    with c3:
        st.write("")
        st.write("")
    ...
    if uploaded and do_extract:
        ...
        try:
            ...
            # (Optionales Preprocessing könntest du hier ergänzen)
